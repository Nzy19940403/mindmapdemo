...
let MindMapComponent = class MindMapComponent {
    constructor() {
        this.mindmapData = [];
    ...
    initNode(node) {
        ...
        let _this = this;
        let drawtype = d3__WEBPACK_IMPORTED_MODULE_2__["area"]()
            .x0(function (d) {
            return 0;
        })
            .x1(function (d) {
        ...
        wrappernode
            ...
            // debugger
            if (data.children) {
                let t = d3__WEBPACK_IMPORTED_MODULE_2__["select"](this.parentNode).selectAll('g')
            ...
            let t = d3__WEBPACK_IMPORTED_MODULE_2__["select"](this.parentNode).attr('transform').split(',');
            ...
            let t = d3__WEBPACK_IMPORTED_MODULE_2__["select"](this.parentNode);
    ...
    changeMindMapData(parentdata, selfdata, selfDirectParentdata) {
        ...
        //self的祖先parent
        let parent = lodash__WEBPACK_IMPORTED_MODULE_3__["find"](this.mindmapData, { id: parentid });
    ...
    getSelfFromTree(data, targetid, target = null) {
        if (!target) {
            if (lodash__WEBPACK_IMPORTED_MODULE_3__["isArray"](data)) {
                let t = lodash__WEBPACK_IMPORTED_MODULE_3__["find"](data, { id: targetid });
                if (t) {
                    target = t;
                }
                else {
    ...
    doFormatArrayData(data) {
        //格式化数据
        let target = [];
        let tree = d3__WEBPACK_IMPORTED_MODULE_2__["cluster"]();
        // data.forEach((item)=>{
...
let MymindmapComponent = class MymindmapComponent {
    ...
    initComparePositionWatcher() {
        ...
        this.positionCompareTrigger$
            .pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_6__["debounce"])(() => Object(rxjs__WEBPACK_IMPORTED_MODULE_5__["interval"])(300)), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_6__["filter"])(() => {
            ...
            .subscribe(res => {
            let _this = this;
            let list = d3__WEBPACK_IMPORTED_MODULE_2__["select"]('svg.mindmapsvg').selectAll('g.draggingwrapper');
    ...
    initMindmapData(target) {
        //在此处理一下mindmap的数据，下一步再进行渲染
        this.countMindmapLinks();
        let array = [];
        let linksArray = [];
        target.forEach((item) => {
            let root = d3__WEBPACK_IMPORTED_MODULE_2__["hierarchy"](item);
            let list = root.links();
    ...
    countOutputAndImportData(data, type) {
        if (type == 'vertical') {
            //垂直方向
            if (lodash__WEBPACK_IMPORTED_MODULE_3__["isArray"](data)) {
                data.forEach(element => {
    ...
    initMindmapSvg(target) {
        ...
        let svg = d3__WEBPACK_IMPORTED_MODULE_2__["select"]('svg.mindmapsvg')
        ...
        d3__WEBPACK_IMPORTED_MODULE_2__["select"]('svg.mindmapsvg')
            ...
            let mynode = d3__WEBPACK_IMPORTED_MODULE_2__["select"](this)
                .selectAll('g')
            ...
            wrapper
                ...
                // console.log('drag')
                let t = d3__WEBPACK_IMPORTED_MODULE_2__["select"]('svg.dragSurface')
                    .attr('transform').split(',');
                // let clientx = Number(t[0].split('(')[1])
                // let clienty = Number(t[1].split(')')[0])    
                let dragsurfaceCoor = d3__WEBPACK_IMPORTED_MODULE_2__["mouse"](_this.gcontainer.nativeElement);
    ...
    initMindmapLinks(data) {
        ...
        let linktype = d3__WEBPACK_IMPORTED_MODULE_2__["linkVertical"]()
            .x(d => {
            // debugger
            return d.x;
        })
    ...
    initDragSurface() {
        ...
        let array = [];
        let root = d3__WEBPACK_IMPORTED_MODULE_2__["hierarchy"](this.currentDraggingData);
        array.push(root);
        let svg = d3__WEBPACK_IMPORTED_MODULE_2__["select"]('svg.dragSurface')
            .selectAll('g.block')
    ...
    handleAddRelativeCoor(data, isAncestor, parentcoor) {
        ...
        if (data.children) {
            data.children.forEach(element => {
                let obj = lodash__WEBPACK_IMPORTED_MODULE_3__["cloneDeep"](data.position);
                let coor = {
    ...
    rewriteMindmapData(data, obj) {
        ...
        if (this.currentAboutToDropTargetId) {
            ...
            if (formatnodelist.length > 1) {
                ...
                parent.children.push(lodash__WEBPACK_IMPORTED_MODULE_3__["cloneDeep"](this.currentDraggingData));
            }
            else {
                //祖先就是自己
    ...
    doCheckDataChange(checkBeforeEnd) {
        //检查是否可以变更mindmap数据
        ...
        if (checkBeforeEnd) {
            ...
            let compareData = lodash__WEBPACK_IMPORTED_MODULE_3__["cloneDeep"](this._mindmapdata);
...
MymindmapComponent.ctorParameters = () => [
...
let MytestComponent = class MytestComponent {
    constructor() { }
    ngOnInit() {
    }
};
...
let TestmindComponent = class TestmindComponent {
    constructor() {
        this.mindmapdata = [
            {
                text: '123',
    ...
    ngOnInit() {
        ...
        this.mindmapdata.forEach((d) => {
            let root = d3__WEBPACK_IMPORTED_MODULE_2__["hierarchy"](d);
            target.push(root);
        });
        this.initsvg(target);
    }
    initsvg(target) {
        ...
        d3__WEBPACK_IMPORTED_MODULE_2__["select"]('svg.mindmapsvg')
            ...
            mynode
                ...
                .on('drag', function (data) {
                // let t = D3.select(this.parentNode)
                let t = d3__WEBPACK_IMPORTED_MODULE_2__["select"]('svg.movingsvg')
                    .attr('transform').split(',');
                let clientx = Number(t[0].split('(')[1]);
                let clienty = Number(t[1].split(')')[0]);
    ...
    temporaryChange(selfdata) {
        ...
        let ancestorsparent = lodash__WEBPACK_IMPORTED_MODULE_3__["find"](this.mindmapdata, { id: nodelist[nodelist.length - 1].data.id });
    ...
    initMovingMindmapSvg() {
        ...
        let clienx = lodash__WEBPACK_IMPORTED_MODULE_3__["sortBy"](array, 'x')[0].x;
        let clieny = lodash__WEBPACK_IMPORTED_MODULE_3__["sortBy"](array, 'y')[0].y;
        d3__WEBPACK_IMPORTED_MODULE_2__["select"]('svg.movingsvg').attr('transform', `translate(${clienx},${clieny})`);
        let svg = d3__WEBPACK_IMPORTED_MODULE_2__["select"]('svg.movingsvg')
            .selectAll('g.block')
